...
export function CookieLogin({ onSuccess, scope = 'cn' }: CookieLoginProps) {
    ...
    const handleSubmitCookie = async () => {
        if (!cookieToken.trim()) {
            setError('请输入 cookie_token_v2');
            return;
        }
        if (!accountId.trim()) {
            setError('请输入 account_id');
            return;
        }
        ...
        try {
            ...
            const cookie = `cookie_token_v2=${cookieToken.trim()}; account_id=${accountId.trim()}`;
    ...
    const cookieInstructions = scope === 'global' ? [
        ...
        '5. 分别复制 cookie_token_v2 和 account_id 的值',
        '6. 粘贴到下方对应的输入框中'
    ] : [
        '1. 在弹出的米游社窗口中完成登录',
        '2. 登录成功后，按 F12 打开开发者工具',
